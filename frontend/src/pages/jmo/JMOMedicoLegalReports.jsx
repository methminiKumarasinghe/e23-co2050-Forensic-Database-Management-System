...
import { getMlrReports } from '../../api/jmo.api';
...
const JMOMedicoLegalReports = () => {
  ...
  const fetchReports = async () => {
    try {
      const data = await getMlrReports();
      setReports(Array.isArray(data) ? data : []);
    } catch (err) {
      console.error('Failed to load MLR reports', err);
    } finally {
      setLoading(false);
    }
  };
      ...
      <main className="pt-16 max-w-screen-xl mx-auto px-4 sm:px-6 py-8">
        <div className="mb-6 flex justify-between items-center">
          <div>
            <div className="text-xs text-emerald-400 mb-1">
              <Link to="/dashboard/jmo" className="hover:underline">Dashboard</Link> &gt; Medico-Legal Reports
            </div>
        ...
        {loading ? (
          ...
          <div className="glass rounded-xl p-8 text-center text-gray-400 border border-gray-800">
            <p>No Medico-Legal Reports created yet. Complete an MLEF examination to generate report drafts.</p>
          </div>
        ) : (
              ...
              <div key={r.report_id} className="glass rounded-xl p-5 border border-gray-800 space-y-3">
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Link to={`/dashboard/jmo/mlef/${r.mlef_id}/report`} className="btn-secondary text-xs py-1 px-3">
                      View Full MLR
                    </Link>
                  </div>
                </div>

                <div className="text-xs text-gray-300 bg-gray-950/60 p-4 rounded border border-gray-800 space-y-2">
                  <p><span className="text-gray-400 font-semibold">Prepared Date:</span> {new Date(r.prepared_date).toLocaleString()}</p>
                  <p><span className="text-gray-400 font-semibold">Medical Opinion:</span> {r.medical_opinion || 'N/A'}</p>
                  <p><span className="text-gray-400 font-semibold">Recommendations:</span> {r.recommendations || 'N/A'}</p>
                </div>
              </div>
            ))}
          </div>
